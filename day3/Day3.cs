...
public record struct Entry(int Top, int Left, string Contents);

public record struct BoundaryBox(int Left = 0, int Top = 0, int Right = 0, int Bottom = 0);
...
public static class Day3
{
    ...
    public static IEnumerable<(int, int)> GetBorderPoints(this Entry entry, BoundaryBox boundary) =>
        entry.GetBorders().Clip(boundary);

    public static IEnumerable<(int, int)> Clip(this BoundaryBox boundary, BoundaryBox other) =>
    ...
    public static IEnumerable<(int, int)> GetPoints(this BoundaryBox boundary)
    {
